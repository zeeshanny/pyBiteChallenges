"""
In this Bite you will use ElementTree to parse some Nolan movies we extracted
from OMDb.

Luckily most APIs switched to JSON, but sometimes XML is all there is, so at
least learnto parse some! Complete the get_tree, get_movies and
get_movie_longest_runtime functions below. See the docstrings and tests for
more info.
"""
...
def get_movies():
    """Call get_tree and retrieve all movie titles, return a list or 
    generator"""
    tree = get_tree()
    ...
    # root = get_tree()
    # movieList = [movie.get('title') for movie in root.findall('movie')]
    # return movieList


def _get_runtime(movie): # helper function created for below method
...
def get_movie_longest_runtime():
    ...
    # root = get_tree()
    # movie_runtime = max([movie.get('runtime') for movie in root.findall('movie')])
    # for elem in root.iterfind('movie[@runtime="' + movie_runtime + '"]'):
